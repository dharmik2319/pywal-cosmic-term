...
parser: argparse.ArgumentParser = argparse.ArgumentParser("Use your pywal colorscheme with COSMIC Terminal")
parser.add_argument('--pywal-colors', type=pathlib.Path, nargs=1, help='Path to colors file generated by pywal', default=pathlib.Path(os.environ['HOME']+'/.cache/wal/colors'), required=False)
parser.add_argument('--sample-ron', type=pathlib.Path, nargs=1, help='Path to a .ron file for COSMIC Terminal (Either export one or download one', required=True)
parser.add_argument('--output', type=pathlib.Path, nargs=1, help='Path to output file', default=pathlib.Path('pywal.ron'), required=False)

args: argparse.Namespace = parser.parse_args()

ron: Dict[str, str|Dict[str, str]] = pyron.load(path=str(args.sample_ron[0].absolute()))
...
breakpoint()
ron_ls: List[str] = pyron.to_string(ron).replace('{', '(').replace('}',')').split('\n')
for line in range(len(ron_ls)):
    ron_ls[line] = ron_ls[line].replace('"','',2)
